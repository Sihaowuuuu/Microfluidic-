...
def count_channel(image_path):
    ...
    cleaned_image = morphology.remove_small_holes(
        cleaned_image, area_threshold=10
    ...
    local_max = peak_local_max(
        distance_map, min_distance=20, footprint=np.ones((3, 3)), labels=cleaned_image
    )
...
def batch_process_images(input_folder, excel_path):
    ...
    final_df = pd.concat(
        [
            final_df,
            pd.DataFrame(
                [{"image_name": "Total", "particle_count": total_particle_count}]
            ),
        ]
    )
