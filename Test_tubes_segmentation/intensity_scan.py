            ...
            for filename in sorted(files):
                ...
                if "ch00" in filename and "t00" in filename:
                    ...
                    if low_intensity_values1.size > 0:
                        ...
                        low_intensity_values2 = low_intensity_values1[
                            low_intensity_values1 < mean2
                        ]

                        # Ensure that the second screening result is not null
                        final_mean = (
                            np.mean(low_intensity_values2)
                            if low_intensity_values2.size > 0
                            else mean2
                        )
                    else:
                    ...
                    while not found:
                        if y < 0 or y >= height:
                            if direction == "down":
                                ...
                                print(
                                    f"Switching to upward search for channel 0: {filename}"
                                )
                                y = round(height / 2) - 70
                                direction = "up"
                                continue
                                ...
                                print(
                                    f"Unable to find sufficient low-intensity points in channel 0: {image_path}"
                                )
                                overlay_coordinates = None
                                break
                        ...
                        overlay_coordinates = find_reasonable_low_intensity_pixels(
                            intensity_values,
                            target_low_intensity_count,
                            final_mean + final_mean / 5,
                        )
        ...
        for root, dirs, files in os.walk(
            ...
            for filename in sorted(files):
                ...
                output_dir = os.path.join(
                    save_folder, f"Segmented_{image_name}_1024"
                ...
                for i in range(0, len(overlay_coordinates) - 1, 2):
                    ...
                    output_filename = (
                        f"{i // 2}.tif"  # Add 1024 suffix to the output filename
                    )
                    ...
                    try:
                        ...
                        if not success:
                            ...
                            print(
                                f"WARNING: TIFF failed to save, converted to PNG format: {png_path}"
                            )
                        else:
    ...
    for root, dirs, files in os.walk(
        ...
        for filename in sorted(files):
            ...
            if "ch01" in filename.lower():
                ...
                save_folder = os.path.join(output_folder, time_name)
                output_dir = os.path.join(
                    save_folder, f"Thresholded_Segmented_{image_name}_1024"
                )  # Add '_1024' suffix to the image name
                os.makedirs(output_dir, exist_ok=True)
                ...
                for i in range(0, len(overlay_coordinates) - 1, 2):
                    ...
                    roi = image[:, start_x:end_x]
                    output_filename = (
                        f"{i // 2}.jpg"  # Add 1024 suffix to the output filename
                    )
                    output_path = os.path.join(output_dir, output_filename)
                    cv2.imwrite(
                        output_path, (roi * 255).astype(np.uint8)
                    )  # Save the cropped image in color
                    print(
                        f"Processing complete. Processed images saved in {output_dir}"
                    )
